...
function benchmark_mapf(map_matrix::Matrix{Char}, scenario::DataFrame; bucket=1)
...
function benchmark_mapf(map_path::AbstractString, scenario_path::AbstractString; bucket=1)
    map_matrix = read_benchmark_map(map_path)
    scenario = read_benchmark_scenario(scenario_path)
    return benchmark_mapf(map_matrix, scenario; bucket=bucket)
end
